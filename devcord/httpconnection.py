...
class HTTPClient:
    ...
    Thanks to @angelCarias (github) for this code! <3   
    """
    ...
    async def login(self):
        ...
        self.session = aiohttp.ClientSession(headers={
            ...
            "User-Agent": f"pycordia @ v{devcord.__version__} on {platform.platform()}"
        })

    async def create_multipart(self, data: Any, files: List['devcord.models.File']) -> bytes:
    ...
    async def request(
        ...
        files: List['devcord.models.File'] = None
        ...
        if not self.session:
            raise devcord.errors.ClientSetupError("No HTTP client session found.")
        ...
        async with self.session.request(
            ...
            **kws, headers={ "Content-Type": content_type }
            ...
            if not resp.ok:
                raise devcord.errors.determine_error(resp.status, rs)
